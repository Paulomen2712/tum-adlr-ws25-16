...
from model.environments import LunarLanderWithKnownWind
...
class PPO:
    ...
    def train(self, total_timesteps):
        ...
        while t_sim < total_timesteps:   
            obs, acts, log_probs, ep_lens, advantages, modified_obs = self.rollout()
            ...
            returns = advantages + self.critic(modified_obs).squeeze().detach()
    ...
    def rollout(self):
        ...
        return batch_obs, batch_acts, batch_log_probs, batch_lens, batch_advantages.squeeze(), batch_modified_obs
    ...
    def get_action(self, obs):
        ...
        return action.detach().numpy(), log_prob.detach(), values.detach(), modified_obs
    ...
    def evaluate(self, batch_obs, batch_acts):
        ...
        return values.squeeze().clone(), log_probs
    ...
    def validate(self, max_iter, should_record=False, env_class=LunarLanderWithKnownWind):
        self.policy.eval()
        if should_record:
            env = env_class()
    ...
    def test(self, env_class=LunarLanderWithKnownWind):
        self.policy.test()
        env = env_class(render_mode='human')
        while True:
                obs, done = env.reset()
